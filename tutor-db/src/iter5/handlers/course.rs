...
pub async fn update_course(
  ...
  let (tutor_id, course_id) = path.into_inner();
  update_course_db(&app_state.db, tutor_id, course_id, update_course.into())
    .await
    .map(|course| HttpResponse::Ok().json(course))
}
...
mod tests {
  ...
  async fn post_course_success() {
    ...
    let new_course_msg = CreateCourse {
      tutor_id: 1,
      course_name: "This is the next auto-test course".into(),
  ...
  async fn update_course_success() {
    ...
    let resp = update_course(app_state, update_param, params)
      .await.unwrap();
    assert_eq!(resp.status(), StatusCode::OK);
  }
