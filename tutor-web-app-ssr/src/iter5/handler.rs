...
) -> Result<HttpResponse, Error> 
{
    ...
    if user_not_found {
        if params.password != params.confirmation {
            ...
            // println!(">>> pre awc_client {:?}", &new_tutor);
...
credentials.",
            ...
            let hash = 
                argon2::hash_encoded(params.password.clone().as_bytes(), salt, &config)
                .unwrap();
            let user = User {
                username,
